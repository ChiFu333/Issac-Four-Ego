...
namespace Editor.CMSEditor
{
    public class CMSSelectorPopup : EditorWindow
    {
        ...
        private void DrawSelectableRow(CMSEntityPfb prefab)
        {
            ...
            var rect = GUILayoutUtility.GetRect(new GUIContent(displayName), EditorStyles.label, GUILayout.ExpandWidth(true));
            ...
            EditorGUI.LabelField(labelRect, displayName);  // Используем displayName вместо prefab.name
        ...
        private string GetDisplayName(CMSEntityPfb prefab)
        {
            string path = AssetDatabase.GetAssetPath(prefab.gameObject);
    
            // Убираем "Assets/Resources/CMS/" и ".prefab"
            path = path
                .Replace("Assets/Resources/CMS/CardsData/", "")
                .Replace(".prefab", "");
            ...
            realPath += "_";
            realPath = realPath.Replace("__", ")");
            return realPath + " " + parts[^1];
    
            // Если нет подпапок, возвращаем просто имя
            return prefab.name;
        }
